...
private:
  ...
  Node* insert(const value_type &newValue);

  //helper function for the insert function
  void insertHelper(Node* source, Node* newNode);

  //helper function for the inorder function
  void inorderHelper(Node* source, void visit(const value_type &item)) const;

  //helper function for the preorder function
  void preorderHelper(Node* source, void visit(const value_type &item)) const;

  //helper function for the postorder function
  void postorderHelper(Node* source, void visit(const value_type &item)) const;

  //helper function for the copy constructor
  Node* copyNode(Node* source);

  //helper function for the clear function
  void clearHelper(Node* source);

  //helper function for the contains function
  bool containsHelper(const key_type &key, Node* source) const;

  //helper function for the operator[]
  value_type* getValue(const key_type &key, Node* source) const;

  //helper function for the == and != function
  bool equalsHelper(Node* one, Node* two) const;

  //helper function for the rebalance function
  void rebalanceHelper(Node* source, vector<value_type>& rebalanceVector);

  //helper function for the rebalance vector node
  Node* reinsertionHelper(vector<value_type>& rebalanceVector, int start, int length);

  //helper function for the getAll function
  void getAllHelper(vector<value_type>& v, Node* source,const key_type &k) const;
};
